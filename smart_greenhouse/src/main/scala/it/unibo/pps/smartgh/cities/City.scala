...
trait City:
  ...
  /** @return city's environment values, example of returned value:
   *(location, HashMap(name -> Rome,
   *  country -> Italy,
   *  tz_id -> Europe/Rome,
   *  region -> Lazio,
   *  lon -> 12.48,
   *  localtime -> 2022-07-27 9:11,
   *  localtime_epoch -> 1658905896,
   *  lat -> 41.9))
   *  (current,HashMap(temp_f -> 78.8,
   *    uv -> 7.0,
   *    vis_km -> 10.0,
   *    feelslike_c -> 26.9,
   *    gust_kph -> 13.7,
