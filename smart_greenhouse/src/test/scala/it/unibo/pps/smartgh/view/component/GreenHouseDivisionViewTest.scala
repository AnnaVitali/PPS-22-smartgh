...
class GreenHouseDivisionViewTest extends AbstractViewTest:
  ...
  @Test def testStartAreaColor(robot: FxRobot): Unit =
    val initial =
      if areaModel.sensors.forall(s => s.status == SensorStatus.NORMAL) then normalStateClassStyle
  ...
  private def areaStateWithAllOtherNORMAL(): Unit =
    ...
    areaModel.status =
      if areaModel.sensors.forall(s => s.status == SensorStatus.NORMAL) then AreaStatus.NORMAL else AreaStatus.ALARM
